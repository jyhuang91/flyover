...
def main():
    ...
    for s, scheme in enumerate(schemes):
        for o, off in enumerate(off_percentile):
            ...
            zeroload_lat = 0
            latency_breakdown[0][o * len(schemes) + s] = hops[s][o] * 3                     # router_lat
            zeroload_lat += latency_breakdown[0][o * len(schemes) + s]
            latency_breakdown[1][o * len(schemes) + s] = flov_hops[s][o]                    # flov_lat
            zeroload_lat += latency_breakdown[1][o * len(schemes) + s]
            latency_breakdown[2][o * len(schemes) + s] = flov_hops[s][o] + hops[s][o] + 1   # link_lat
            ...
            latency_breakdown[4][o * len(schemes) + s] = latency[s][o] - zeroload_lat       # contention

    paper_latency = latency[[0, 1, 3, 4, 5, 6], :]
    paper_dynamic_power = dynamic_power[[0, 1, 3, 4, 5, 6], :]
    paper_static_power = static_power[[0, 1, 3, 4, 5, 6], :]
    paper_total_power = total_power[[0, 1, 3, 4, 5, 6], :]
    ...
    figname = traffic + injection_rate + 'latency.pdf'
    ...
    figname = traffic + injection_rate + 'dynamic_power.pdf'
    pdfpage, fig = pdf.plot_setup(figname, figsize=(8, 4), fontsize=14)
    ax = fig.gca()
    for s, scheme in enumerate(paper_schemes):
        ax.plot(
    ...
    figname = traffic + injection_rate + 'total_power.pdf'
    pdfpage, fig = pdf.plot_setup(figname, figsize=(8, 4), fontsize=14)
    ax = fig.gca()
    for s, scheme in enumerate(paper_schemes):
        ax.plot(
    ...
    #figname = traffic + injection_rate + 'static_power.pdf'
    ...
    figname = traffic + injection_rate + 'lat_breakdown.pdf'
    ...
    for pos in xrange(ly + 1):
        lxpos = (pos + 0.5) * scale
        if pos < ly:
            ax.text(lxpos, ypos, off_percentile[pos], ha='center', transform=ax.transAxes)
