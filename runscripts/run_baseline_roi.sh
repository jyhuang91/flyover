...
logfile=log/baseline/${benchmark}${numthread}_${meshrows}x${meshrows}mesh_${datasize}.log
mkdir -p log/baseline

./build/${system}/gem5.fast \
  -d m5out/restore_baseline/${benchmark}${numthread}_${meshrows}x${meshrows}mesh_sim${datasize} \
  ...
  --booksim-network \
  --noc-pg \
  --booksim-config=configs/ruby_booksim2/gem5booksim.cfg \
  >  ${logfile} 2>&1 &
