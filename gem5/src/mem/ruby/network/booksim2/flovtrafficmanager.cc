    ...
    if ( f->tail ) {
        ...
        if ( f->watch ) {
            *gWatchOut << GetSimTime() << " | "
                       ...
                       // NB: In the spirit of solving problems using ugly
                       // hacks, we compute the packet length by taking
                       // advantage of the fact that the IDs of flits within a
                       // packet are contiguous.
                       << ", src = " << head->src
                       << ", dest = " << head->dest
                       << ")." << endl;
        }
                                      ...
                                      int cl, uint64_t time )
{
    ...
    for ( int i = 0; i < size; ++i ) {
        ...
        case class_based:
            ...
            break;
        case age_based:
            f->pri = numeric_limits<uint64_t>::max() - time;
            assert(f->pri >= 0);
            break;
        case sequence_based:
            f->pri = numeric_limits<uint64_t>::max() - _packet_seq_no[source];
            assert(f->pri >= 0);
            break;
        default:
            f->pri = 0;
