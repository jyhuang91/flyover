...
RoutingTable::RoutingTable(int source, int num_vertices,
		vector<bool> router_states, vector<vector<int> > adj_matrix) {
	_source = source;
	...
	_distance.resize(_num_vertices, INFINITY);
	_distance[_source] = 0;
}

vector<vector<int> > RoutingTable::_GetAdjMatrix() {
	return _adj_matrix;
}
...
#ifdef DEBUG
		cout << "closest unmarked node is " << closestUnmarkedNode << ", marked? " << _mark[closestUnmarkedNode] << endl;
#endif
		_mark[closestUnmarkedNode] = true;
		for (int i = 0; i < _num_vertices; i++) {
			if ((!_mark[i]) && (_adj_matrix[closestUnmarkedNode][i] > 0)) {
				if (_distance[i]
						> _distance[closestUnmarkedNode]
...
#ifdef DEBUG
	string direction_vec[6] = { "INVALID", "WEST", "EAST", "SOUTH",
			"NORTH", "ARRIVED" };
...
vector<int> RoutingTable::GetRoutingTbl() {
	return _routing_tbl;
}
...
// calculate the distance for the new spanningAdjMatrix for up down routing table construction.
...
// Function to calculate routing table with new spnningAdjMatrix for up down routing.
void RoutingTable::spanning_routing_tbl_calculate() {
	...
	cout << "destination\toutport" << endl;
	for (int i = 0; i < _num_vertices; i++)
		cout << "  " << i << "\t\t" << direction_vec[_escape_routing_tbl[i]+1] << endl;
	cout << "exit function escape routing table calculate \n";
#endif
}

// Function called from topology2Dmesh class in order to construct the secondary routing table for escape channel.
vector<int> RoutingTable::BuildEscapeRoutingTable(
		int fabric_manager) {
...
// To compute the spanningAdjMatrix for spanning tree constructed from cyclic graph. takes adjMatrix as input. and gives out spanningAdjMatrix.
...
//    printf("SPNNING TREE: spanning matrix resized \n");
	...
	// Initializing all values in spanning adjacent matrix to -1(actualy infinite).
